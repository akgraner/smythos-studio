...
import { Agent } from '@src/react/shared/types/agent-data.types';
...
interface IVoiceDialogProps {
  isOpen: boolean;
  closeModal: () => void;
  currentData: { id: string; properties: TVoiceProperties };
  activeAgent: Agent;
  agentId: string;
}
...
const VoiceDialog = ({
  ...
  useEffect(() => {
    ...
    const _activeData = mapVoiceEmbodimentProperties(properties, activeAgent);
            ...
            >
              <div className="w-[80vw] max-w-[600px]">
                <Dialog.Panel className="w-full relative transform overflow-hidden rounded-xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  >
                    {(props: FormikProps<VoiceEmbodimentData>) => {
                      return (
                        <Form>
