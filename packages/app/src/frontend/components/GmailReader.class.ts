...
export class GmailReader extends Component {
  ...
  private async gmailConnectClick(sender, state) {
    ...
    let retries = 5;
    this._conCheckInterval = setInterval(async () => {
      ...
      if (state && state.refresh_token && state.account) {
        clearInterval(this._conCheckInterval);
        sender.parentElement.querySelector('[name="account"]').value = state.account;
        sender.parentElement.querySelector('[name="token"]').value = state.refresh_token;
      }
