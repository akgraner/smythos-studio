...
import { AccessCandidate, ConnectorService, Logger, ModelsProviderConnector } from '@smythos/sre';
...
async function getCustomModels(teamId: string): Promise<Record<string, any>> {
  const modelsProviderConnector: ModelsProviderConnector = ConnectorService.getModelsProviderConnector();
  const modelProviderCandidate = modelsProviderConnector.requester(AccessCandidate.team(teamId));
...
async function getModels(): Promise<Record<string, any>> {
  const modelsProviderConnector: ModelsProviderConnector = ConnectorService.getModelsProviderConnector();
  const modelProviderCandidate = modelsProviderConnector.requester(AccessCandidate.user('system-smyth'));
