...
export default async function agentLoader(req, res, next) {
  ...
  const agentFromModel: any = isAgentLLMCall
    ? getAgentIdAndVersion(req.body.model)
    : {};
  ...
  let agentVersion =
    req.header("X-AGENT-VERSION") || agentFromModel.version || "";
  ...
  if (agentId) {
    ...
    if (
      isAgentChatRequest &&
      (path.startsWith("/stream") || path.startsWith("/chat-stream"))
    ) {
    ...
    req._agent.domain = isAgentLLMCall
      ? "AgentLLM"
      : agentDomain || (await getAgentDomainById(agentId));
