import {
  ...
  IChatStorage,
  IMetadataFile,
} from "@/modules/embodiment/types/chat.types";
...
export class ChatDataManager {
  ...
  constructor({
    ...
    if (!this.teamId || !this.userId || !this.agentId || !this.conversationId) {
      throw new Error(
        "teamId, userId, agentId and conversationId are required"
      );
    }
  ...
  async pushMessage(
    ...
    if (enforceUnique && chunkFile.length > 0) {
      ...
      if (
        mostRecentMessage.role === newMessage.role &&
        mostRecentMessage.content === newMessage.content
      ) {
  ...
  async getContextMessages(tokenWindow: number): Promise<IChatMessage[]> {
    ...
    while (totalContentLength < maxContentLength && i >= 0) {
      ...
      messages.unshift(
        ...prevChunk.map((msg) => this.removeOptionalProps(msg))
      );
      totalContentLength +=
        (JSON.stringify(prevChunk).length - prevChunk.length * 33) / 4; // approximating the token length
      i--;
    }
