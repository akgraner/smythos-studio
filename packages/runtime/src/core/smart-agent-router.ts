...
/**
 * Determines which service should handle the request based on explicit headers
 * This provides stable, predictable routing that doesn't depend on domain names
 *
 ...
 * 4. Deployed versions: Always use agent-runner for production safety
 * 5. X-ROUTING-MODE: Explicit mode selection (debugger|agent-runner)
 * 6. Default: agent-runner (production-safe fallback)
 *
 * USAGE EXAMPLES:
...
function shouldUseDebugger(req: any): { useDebugger: boolean; reason: string } {
  ...
  if (!agent) {
    return { useDebugger: false, reason: "No agent context" };
  }
  ...
  // 4. Check if it's a deployed version (production) - always use agent-runner
  const isDeployedVersion =
    agent.agentVersion &&
    agent.agentVersion !== "development" &&
    agent.agentVersion !== "latest";

  if (isDeployedVersion) {
    return {
      useDebugger: false,
      reason: `Deployed version: ${agent.agentVersion}`,
    };
  }

  // 5. Default routing based on explicit mode header
  ...
  // 6. Final fallback: Default to agent-runner for production safety
  // This ensures that without explicit headers, we always use the production-safe route
  return {
    useDebugger: false,
...
export function createSmartAgentRouter(
  ...
  const smartMiddleware = async (req: any, res: express.Response) => {
    ...
    try {
      // Step 1: Load agent context using combined middleware approach
      await runMiddleware(req, res, dependencies.agentRunner.uploadHandler);
      await runMiddleware(req, res, dependencies.debugger.agentLoader);
 ...
 */
export function testRoutingDecision(
  agentContext: any,
  headers: Record<string, string> = {}
): { useDebugger: boolean; reason: string } {
  const mockReq = {
    _agent: agentContext,
