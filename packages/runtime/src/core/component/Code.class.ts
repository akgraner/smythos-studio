import { Agent, Component, TemplateStringHelper } from "@smythos/sre";
...
export class Code extends Component {
  ...
  async process(input, config, agent: Agent) {
    ...
    try {
      ...
      const url = process.env.CODE_SANDBOX_URL + "/run-js";
      ...
      if (config.data._templateVars) {
        ...
        code_body = TemplateStringHelper.create(code_body).parse(
          config.data._templateVars
        ).result;
      }
